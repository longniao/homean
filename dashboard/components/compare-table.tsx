  ...
  return (
    <div className="overflow-x-auto rounded-2xl border bg-white">
      <table className="min-w-[900px] w-full border-collapse text-left text-sm">
        ...
        <tbody>{rows.map((row) => <tr className="border-t align-top" key={row.key}><th className="sticky left-0 bg-white p-4 font-semibold">{row.label}</th>{showings.map((showing) => <td className="border-l p-4 leading-6 text-stone-600" key={showing.id}>{row.render(showing)}</td>)}</tr>)}</tbody>
      </table>
    </div>
  );
}
