    ...
    <div>
      <div className="mb-10 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-5 max-w-2xl text-base leading-7 text-[#595e69]">{t("subtitle")}</p>
        </div>
        {billing.data?.active === false ? (
          ...
          <Button className="h-12 shrink-0 rounded-full px-6 text-sm font-bold shadow-[0_8px_24px_rgb(34_35_41_/_0.18)] transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_12px_32px_rgb(34_35_41_/_0.24)]" nativeButton={false} render={<Link href="/showings/new" />}><Plus /> {t("newShowing")}</Button>
        )}
      </div>
      ...
      <div className="panel mb-10 p-4 sm:p-5">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3 border-b border-[#e5e8ee] pb-4">
          ...
          <span className="flex items-center gap-1.5 text-xs font-medium text-[#667080]">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
          <label className="relative xl:col-span-2">
            ...
            <Search className="pointer-events-none absolute left-3 top-3.5 size-4 text-[#707886]" />
            <input
          ...
          <div className="relative">
            ...
            <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[#707886]">▾</span>
          </div>
          <div className="relative">
        ...
        <>
          ...
          {groups.length === 0 ? (
            <div className="panel relative flex min-h-96 flex-col items-center justify-center overflow-hidden p-8 text-center">
              <div aria-hidden="true" className="absolute -right-24 -top-24 size-64 rounded-full border border-[#222329]/6" />
              <div aria-hidden="true" className="absolute -right-10 -top-10 size-36 rounded-full border border-[#222329]/6" />
              <div aria-hidden="true" className="absolute -left-16 bottom-0 size-48 rounded-full border border-[#354f79]/5" />
              <div className="mb-6 rounded-full bg-[#b4c9ed] p-5 text-[#222329] shadow-[0_8px_24px_rgb(180_201_237_/_0.4)]">
                <CalendarDays className="size-7" />
              </div>
              <h2 className="max-w-lg font-serif text-4xl font-medium leading-none tracking-[-0.045em] text-[#222329]">{t("emptyTitle")}</h2>
              <p className="mt-4 max-w-md text-sm leading-6 text-[#595e69]">{t("emptyBody")}</p>
              {billing.data?.active === false ? (
              ...
              {groups.map(([id, group]) => (
                <section key={id}>
                  <div className="mb-4 flex items-center gap-3">
                    <h2 className="font-serif text-xl font-semibold text-[#222329]">{group.label}</h2>
                    <span className="rounded-full bg-[#b4c9ed] px-2.5 py-0.5 text-xs font-bold text-[#222329] shadow-[0_2px_8px_rgb(180_201_237_/_0.5)]">
                      {group.items.length}
                    </span>
                  </div>
                  <div className="grid gap-3 xl:grid-cols-2">
                    {group.items.map((showing) => (
                      <Link
                        className="panel group flex items-center gap-4 p-4 transition duration-200 hover:-translate-y-1 hover:border-[#bec6d1] hover:shadow-[0_20px_50px_rgb(34_35_41_/_0.12)]"
                        href={`/showings/${showing.id}`}
                        key={showing.id}
                      >
                        <div className="flex size-12 shrink-0 items-center justify-center rounded-full bg-[#222329] text-[#b4c9ed] shadow-[0_4px_12px_rgb(34_35_41_/_0.25)] transition duration-200 group-hover:scale-110">
                          <Building2 className="size-5" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <div className="mb-1 flex flex-wrap items-center gap-2">
                            <h3 className="truncate font-semibold transition duration-150 group-hover:text-[#354f79]">
                              {showing.property?.display_name ?? t("unassignedProperty")}
                            </h3>
                          ...
                          <p className="truncate text-sm text-[#595e69]">{showing.property?.address ?? t("unassignedProperty")}</p>
                          <p className="mt-1 text-xs text-[#667080]">
                            {showing.contact?.name ?? t("unassignedClient")} · {new Intl.DateTimeFormat(undefined, { dateStyle: "medium" }).format(tourDate(showing))}
                          </p>
                        </div>
                        <span className="text-sm font-bold text-[#354f79] opacity-0 transition duration-150 group-hover:opacity-100">
                          {common("open")}
                        </span>
                      </Link>
                    ))}
