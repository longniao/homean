...
export function ReportScreen({ visitId, onBack }: { visitId: string; onBack: () => void }) {
  ...
  // fetch failed. Every mutation below needs the server, so a stale report is
  // presented read-only rather than offering actions that cannot succeed.
  ...
  useEffect(() => {
    let active = true;
    void (async () => {
      // Paint the last synced copy first so an offline report opens instantly
      // instead of erroring out.
      const [cachedDetail, cachedConfig] = await Promise.all([readShowingDetail(visitId), readVerticalConfig()]);
      if (!active) return;
      if (cachedDetail) { setDetail(cachedDetail); setStale(true); setLoading(false); }
      if (cachedConfig) setVerticalConfig(cachedConfig);
      ...
      try {
        ...
        if (active && !cachedDetail) Alert.alert(t('common.error'));
      } finally { if (active) setLoading(false); }

      // Labels are best-effort: the key fallback keeps an uncached report readable.
  ...
  return <View style={styles.page}>
    ...
    <ScrollView contentContainerStyle={styles.content}>
      {stale && <Card style={styles.offline}><Text style={styles.offlineText}>{t('report.offline')}</Text></Card>}
      <ReportSection title={t('report.summary')}><Text style={styles.summary}>{report.executive_summary}</Text></ReportSection>
      <ReportSection title={t('report.highlights')}><BulletList bullets={report.highlights} onEdit={stale ? undefined : (index, bullet) => setEdit({ location: { group: 'highlights', index }, value: bullet.text })} /></ReportSection>
      <ReportSection title={t('report.concerns')}><BulletList bullets={report.concerns} onEdit={stale ? undefined : (index, bullet) => setEdit({ location: { group: 'concerns', index }, value: bullet.text })} /></ReportSection>
      <Text style={styles.sectionTitle}>{t('report.rooms')}</Text>{report.room_by_room.map((room, roomIndex) => <Card key={`${room.zone_id}-${roomIndex}`} style={styles.card}><Text style={styles.roomTitle}>{verticalLabel(verticalConfig?.displayLabels.zones, room.zone_type, t('home.untitled'))}</Text><BulletList bullets={room.bullets} onEdit={stale ? undefined : (index, bullet) => setEdit({ location: { group: 'room_by_room', room: roomIndex, index }, value: bullet.text })} /></Card>)}
      <ReportSection title={t('report.followUps')}><BulletList bullets={report.follow_ups} onEdit={stale ? undefined : (index, bullet) => setEdit({ location: { group: 'follow_ups', index }, value: bullet.text })} /></ReportSection>
      <Text style={styles.sectionTitle}>{t('report.observations')}</Text>{detail.observations.filter((item) => item.reviewStatus !== 'dismissed').map((item) => <Card key={item.id} style={[styles.card, item.flags.sensitive ? styles.sensitive : undefined]}>
        ...
        {!stale && <SecondaryButton label={t('report.deleteObservation')} onPress={() => { void dismiss(item.id); }} style={styles.delete} textStyle={styles.deleteText} />}
      </Card>)}
