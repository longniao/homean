  ...
  return (
    <div className={cn("mb-6 flex flex-col gap-4 rounded-2xl border p-4 sm:flex-row sm:items-center", failed ? "border-red-200 bg-red-50" : "border-amber-200 bg-amber-50")}>
      ...
      <div className="flex-1">
        ...
        <p className="mt-0.5 text-sm text-[#595e69]">{failed ? showing.processing_error ?? t("processingFailedBody") : t("processingBody")}</p>
      </div>
      {failed && (
...
function ObservationCard({
  ...
  // Derive a CSS accent class from the observation category
  ...
  return (
    <article className={cn("rounded-2xl border border-[#dce0e6] bg-[#ffffff] p-4 transition-shadow hover:shadow-[0_4px_16px_rgb(34_35_41_/_0.07)]", categoryAccent, sensitive && "border-amber-300 bg-amber-50/50", observation.review_status === "dismissed" && "opacity-55")}>
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <span className="rounded-full border border-[#dce0e6] bg-[#f7f8fa] px-2 py-1 text-xs font-medium text-[#595e69]">{t(`review.${observation.review_status}` as never)}</span>
      ...
      {editing ? (
        ...
        <p className="text-sm leading-6 text-[#454b56]">{observation.content}</p>
      )}
      {!editing && (
        ...
        return (
          <section key={zoneIdKey}>
            <div className="mb-3 flex items-center gap-2">
              ...
              <span className={cn("rounded-full bg-[#b4c9ed] px-2 py-0.5 text-xs font-bold text-[#222329]", "shadow-[0_2px_6px_rgb(180_201_237_/_0.4)]")}>{items.length}</span>
            </div>
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
      <div className="space-y-2">
        {bookmarks.length > 0 && (
          <div className="panel p-4">
            <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-[#595e69]">{t("voiceTags")}</p>
            <div className="flex flex-wrap gap-2">
              {bookmarks.map((bookmark, index) => (
                <button
                  className="rounded-full border border-[#dce0e6] bg-[#ffffff] px-3 py-1.5 text-sm font-semibold text-[#3d5d91] transition hover:border-[#3d5d91]/30 hover:bg-indigo-50"
                  key={bookmark.id}
        ...
        {showing.transcript.map((segment) => (
          <article className={cn("panel scroll-mt-28 p-4 transition", segment.id === targetSegmentId && "border-[#3d5d91] ring-4 ring-[#3d5d91]/10")} id={`segment-${segment.id}`} key={segment.id}>
            <div className="flex gap-3">
              <button aria-label={t("playSegment")} className="mt-0.5 flex size-9 shrink-0 items-center justify-center rounded-full bg-[#222329]/8 text-[#3d5d91] transition hover:bg-[#222329]/14" disabled={!audioUrl} onClick={() => play(segment.id, segment.timestamp_start)} type="button"><Play className="ml-0.5 size-4" /></button>
              <div className="min-w-0 flex-1">
                <div className="mb-2 flex items-center justify-between gap-3 text-xs text-[#667080]">
                  <span>{segment.timestamp_start === null ? t("unknownTime") : new Date(segment.timestamp_start).toISOString().slice(14, 19)}</span>
                  {showing.status === "draft" && editing !== segment.id && <button className="font-semibold text-[#3d5d91]" onClick={() => { setEditing(segment.id); setText(segment.text); }} type="button">{t("correct")}</button>}
                </div>
                {editing === segment.id ? (
                  <div>
                  ...
                  <p className="text-sm leading-6 text-[#454b56]">{segment.text}</p>
                )}
                {segment.original_text && <p className="mt-2 border-l-2 border-[#dce0e6] pl-3 text-xs leading-5 text-[#667080]">{t("original")}: {segment.original_text}</p>}
              </div>
            </div>
          </article>
        ))}
      ...
      <aside className="xl:sticky xl:top-8 xl:self-start">
        <div className="panel p-4">
          <p className="mb-3 text-sm font-semibold text-[#222329]">{t("audioTitle")}</p>
          {audioUrl ? <audio className="w-full" controls ref={audioRef} src={audioUrl} /> : <p className="text-sm text-[#595e69]">{t("audioUnavailable")}</p>}
          <p className="mt-3 text-xs leading-5 text-[#667080]">{t("audioHint")}</p>
        </div>
      </aside>
    </div>
  );
...
export function DeliveryPanel({ showing }: { showing: ShowingDetail }) {
  ...
  return (
    <section className="panel mt-7 overflow-hidden border-emerald-200/60">
      <div className="border-b border-[#dce0e6] bg-[#f7f8fa] p-5 sm:p-6">
        <p className="eyebrow mb-2">{t("eyebrow")}</p>
        <h2 className="font-serif text-2xl font-semibold text-[#222329]">{t("title")}</h2>
        <p className="mt-1 text-sm text-[#595e69]">{t("body")}</p>
      </div>
      <div className="grid gap-5 p-5 sm:p-6 lg:grid-cols-2">
        <form className="rounded-xl border border-[#dce0e6] bg-[#ffffff] p-4" onSubmit={(event) => { event.preventDefault(); send.mutate(); }}>
          <div className="mb-3 flex items-center gap-2 font-semibold text-[#222329]"><Mail className="size-4 text-[#3d5d91]" /> {t("emailTitle")}</div>
          ...
          {emailDelivery?.status === "pending" && <p className="mt-2 text-xs leading-5 text-[#595e69]">{t("emailInProgress")}</p>}
        </form>
        <div className="rounded-xl border border-[#dce0e6] bg-[#ffffff] p-4">
          <div className="mb-3 flex items-center gap-2 font-semibold text-[#222329]"><Link2 className="size-4 text-[#3d5d91]" /> {t("linkTitle")}</div>
          <p className="mb-3 text-sm leading-6 text-[#595e69]">{t("linkBody")}</p>
          ...
          {activeShareLink && (
            <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50/60 p-3">
              ...
              {activePdfUrl && <a className="mt-3 inline-flex min-h-9 w-full items-center justify-center gap-2 rounded-lg border border-[#3d5d91] px-3 text-sm font-semibold text-[#3d5d91] transition hover:bg-indigo-50 focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-[#3d5d91]/30" href={activePdfUrl} onClick={(event) => { if (!isActiveShareLink(activeShareLink, Date.now())) event.preventDefault(); }} rel="noreferrer" target="_blank"><FileDown className="size-4" /> {t("openPdf")}</a>}
            </div>
          )}
          ...
          {showing.status === "sent_to_client" && !activeShareLink && <p className="mt-2 text-xs leading-5 text-[#667080]">{t("alreadyDelivered")}</p>}
        </div>
      </div>
            ...
            {entry.kind === "link" && (
              <div className="flex flex-wrap items-center justify-end gap-2">
                {isActiveShareLink(entry.item, now) && <>
                  ...
                  <a aria-label={t("openLink")} className="text-[#3d5d91]" href={entry.item.url} onClick={(event) => { if (!isActiveShareLink(entry.item, Date.now())) event.preventDefault(); }} rel="noreferrer" target="_blank"><ExternalLink className="size-4" /></a>
                </>}
              </div>
            )}
          </div>
...
export function ShowingWorkspace({ id }: { id: string }) {
  ...
  return (
    <div>
      <Link className="mb-6 inline-flex items-center gap-2 text-sm font-semibold text-[#595e69] transition hover:text-[#222329]" href="/"><ArrowLeft className="size-4" /> {t("back")}</Link>
      <div className="mb-7 flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div className="min-w-0">
          <div className="mb-3 flex flex-wrap items-center gap-3"><StatusBadge showing={showing} /><span className="text-xs text-[#667080]">{new Intl.DateTimeFormat(undefined, { dateStyle: "long" }).format(tourDate(showing))}</span></div>
          <h1 className="page-title truncate">{showing.property?.display_name ?? t("unassignedTitle")}</h1>
          <p className="mt-2 text-[#595e69]">{showing.property?.address ?? t("unassignedBody")}{showing.contact ? ` · ${showing.contact.name}` : ""}</p>
        </div>
        <div className="flex items-center gap-2 text-sm text-[#667080]"><Clock3 className="size-4" /> {showing.ended_at ? t("completed") : t("inProgress")}</div>
      </div>
      ...
      <div className="no-print mb-6 tab-bar w-full justify-start overflow-x-auto" role="tablist">
        {tabs.map((item) => <button aria-selected={tab === item.key} className={cn("tab-btn", tab === item.key && "active")} key={item.key} onClick={() => setTab(item.key)} role="tab" type="button">{item.label}{item.count !== undefined && <span className={cn("rounded-full px-1.5 py-0.5 text-[10px]", tab === item.key ? "bg-[#b4c9ed]/20 text-[#b4c9ed]" : "bg-[#dce0e6] text-[#595e69]")}>{item.count}</span>}</button>)}
      </div>
